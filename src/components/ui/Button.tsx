  ...
  "inline-flex items-center justify-center rounded-full text-base font-bold italic transition-all duration-300 disabled:opacity-50 disabled:pointer-events-none tracking-wide font-brand-heading",
  {
    variants: {
      variant: {
        ...
        primary: "bg-brand-green text-brand-dark border-2 border-brand-green hover:bg-brand-dark hover:text-white hover:border-brand-dark hover:shadow-lg",
        
        // 2. Outline - Transparent BG with configurable border/text
        outline: "bg-transparent border-2 hover:bg-brand-green hover:text-brand-dark",
...
export interface ButtonProps
  ...
  textColor?: "brand-green" | "brand-dark" | "brand-surface" | "white" | "black"; // Text color for outline variant
  borderColor?: "brand-green" | "brand-dark" | "brand-surface" | "white" | "black"; // Border color for primary variant
}

export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, href, children, textColor, borderColor, ...props }, ref) => {
    
    // Build dynamic classes for outline variant
    const outlineClasses = variant === "outline" ? cn(
    ...
    const primaryBorderClass = variant === "primary" && borderColor ? `border-${borderColor}` : "";
